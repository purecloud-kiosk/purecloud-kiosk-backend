...
function storeData(key, data, expireTime){
  return new Promise((resolve, reject) => {
    redisClient.set(key, JSON.stringify(data), 'EX', expireTime, (hmSetError, hmSetResponse) => {
      if(hmSetError)
         reject(hmSetError);
      else
...
class CachingService{
  ...
  getSessionData(key){
    ...
    return new Promise((resolve, reject) => {
      redisClient.get(sessionKey, (error, result) => {
  ...
  getEventData(key){
    ...
    return new Promise((resolve, reject) => {
      redisClient.get(eventKey, (error, result) => {
        var result = JSON.parse(result);
  ...
  removeEvent(key){
    return new Promise((resolve, reject) => {
      redisClient.del("event" + key, (error, exists) => {
        if(error)
          reject(error);
        else
          resolve(exists);
  ...
  getSessionTimeToLive(key){
    return new Promise((resolve, reject) => {
      redisClient.ttl("session" + key, (error, ttl) => {
        if(error)
          reject(error);
        else
          resolve(ttl);

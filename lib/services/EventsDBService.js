...
class EventsDBService{
            ...
            dao.insertCheckIn(manager, function(checkInError, checkInResult){
              if(checkInError){ // if checkIn goes wrong there is an issue, rollback
                dao.removeEvent(eventResult._id, function(){
                  return callback({"error" : "An error occured while creating an event."});
                });
              }
              else{
          ...
          validateEvent(eventData, managerData, function(validationError, validatedEvent){
            ...
            retrieveEvent(eventData.eventID, function(error, event){
              if(error){
                ...
                dao.updateEvent(eventData.eventID, validatedEvent, function(error, result){
                  if(error || result === null){
                    return callback({"event" :  "Event does not exist"});
                  }
                  else{
                    if((event.private == "true" ||  event.private == true) && eventData.private == false){
                      // handle this async
                      dao.removeUnattendedCheckInsByEvent(eventData.eventID, function(error, result){
      ...
      if(error){
        ...
        dao.removeEvent(eventID, function(removeError, removeResult){
          if(removeError){
            ...
            // remove all checkins associated to event
    ...
    setPaginationOptions(options, function(error, limit, page){
      ...
      dao.getPrivateEvents(personID, orgGuid, limit, page, function(error, result){
        if(error){
          ...
          result = result.slice(limit * page, (limit * page) + limit);
          var matchedResults = [];
          for(var i = 0; i < result.length; i++){
  ...
  addPrivateAttendee(eventID, managerData, checkInData, callback){
    validateEventIDAndCheckIn(eventID, managerData, checkInData, function(error, event, validatedCheckInData){
      if(error){
      ...
      else if(event.private === false || event.private == "false"){
        ...
        dao.insertCheckIn(validatedCheckInData, function(insertError, insertResponse){
          if(insertError){
  ...
  addEventManager(eventID, managerData, newManagerData, callback){
    validateEventIDAndCheckIn(eventID, managerData, newManagerData, function(error, event, validatedCheckInData){
      if(error){
        ...
        //  insert the checkIn,
        dao.getCheckIn(validatedCheckInData.person_id, eventID, function(getError, checkIn){
          if(getError || checkIn == null){
            // person is not listed as attendeee, insert them in.
            dao.insertCheckIn(validatedCheckInData, function(insertError, result){
              if(insertError)
  ...
  checkIntoEvent(eventID, managerData, checkInData, callback){
    validateEventIDAndCheckIn(eventID, managerData, checkInData, function(error, event, validatedCheckInData){
      if(error){
        ...
        if(event.private == "false" || event.private == false){ // string because of redis
          validatedCheckInData.checked_in = true;
          validatedCheckInData.timestamp = checkInData.timestamp;
          //  insert the Checkin
          dao.insertCheckIn(validatedCheckInData, function(insertError, insertResult){
            if(insertError){
              // duplicate, so attempt update
              dao.getCheckIn(validatedCheckInData.person_id, eventID, function(updateError, oldCheckIn){
          ...
          dao.getCheckIn(validatedCheckInData.person_id, eventID, function(updateError, checkIn){

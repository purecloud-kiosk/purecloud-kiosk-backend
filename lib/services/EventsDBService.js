...
class EventsDBService{
   ...
   **/
  validateEvent(eventData, creatorData, callback){
    if(eventData.image_url === undefined || eventData.image_url.trim() == ""){
  ...
  createEvent(eventData, creatorData, callback){
    this.validateEvent(eventData, creatorData, function(error, event){
      if(error){
        callback(error);
      }
      else{
  ...
  updateEvent(eventData, managerData, callback){
    if(eventData.eventID === undefined){
      ...
      var self = this;
      self.checkIfManager(managerData, eventData.eventID, function(error, result){
        if(error){
          callback(error);
        }
        else{
          self.validateEvent(eventData, managerData, function(validationError, event){
            /**

...
var inviteErrorTypes = require('lib/models/errors/InviteServiceError').errorTypes;
...
function validateCheckIn(options){
  return new Promise((resolve, reject) => {
    ...
    if(!valid){
      rejectResponse.providedCheckIn = options;
    ...
    if(eventID === undefined || checkIn === undefined){
      ...
      checkIfUserHasAccess(manager, eventID).then(function(){
        ...
        return validateCheckIn({
          'eventID' : event.id,
          'checkIn' : checkIn,
          'requireTimestamp' : options.requireTimestamp
        });
  ...
  bulkCheckIn(options){
    return new Promise((resolve, reject) => {
      ...
      if(checkInArray == null || checkInArray == undefined ||
        !Array.isArray(checkInArray) || checkInArray.length == 0 ){
          ...
          bulkOps.push(new Promise((resolveInner, rejectInner) => {
            ...
            checkIfUserHasAccess(options.user, event.id).then(() => {
              ...
              checkInMap[event.id].forEach((checkIn) => {
                promises.push(validateCheckIn(checkIn));
              });
              Promise.all(promises.map((promise) => {
                ...
                // after processing each promise, resolve bulk op
                var promise;
                if(event.private){

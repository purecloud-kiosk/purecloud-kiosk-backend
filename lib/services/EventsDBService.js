    ...
    this.validateEventIDAndCheckIn(eventID, managerData, checkInData, function(error, event){
      if(error){
        ...
        dao.insertCheckIn(checkIn, function(insertError, insertResult){
          if(insertError){
            // duplicate
            callback({"error" : "User already checked_in"});
          }
          else{
            callback(null, {"res" : "User checked in successfully."});
          }
        });
      }
      else{
        ...
        dao.updateCheckIn(checkInData.personID, eventID, updatedCheckInData, function(updateError, checkIn){

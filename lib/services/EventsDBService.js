...
function validateEvent(eventData, creatorData){
  return new Promise(function(resolve, reject){
    ...
    if(eventData.title === undefined || eventData.title.length == 0 ||
      eventData.startDate === undefined){
        reject(errorResponses.TITLE_OR_DATE_MISSING);
    }
    else{

      console.log(new Date(eventData.startDate));
      console.log(new Date(eventData.endDate));
...
function validateCheckIn(options){
  return new Promise((resolve, reject) => {
    ...
    if(checkIn.personID === undefined || checkIn.personID.trim() === '' ||
      ...
      validatedCheckIn.timestamp = new Date(checkIn.timestamp);
...
class EventsDBService{
  ...
  checkIfPartOfEvent(user, eventID){
    return new Promise((resolve, reject) => {
      retrieveEvent(eventID).then((event) => {
        if(!event.private){
          resolve();
        }
        else{
   ...
   * @param {string} eventID - the id of the event to retrieve
   *
   * @return Returns a promise that resolves with the event. Will reject if the event does not exist or if user has no access
   **/
  fetchEvent(user, eventID){
    console.log('this got called')
    return new Promise((resolve, reject) => {
      let event;
      retrieveEvent(eventID).then((e) => {
        event = e;
        if(!e.private || user.userType === 'admin'){
          return new Promise.resolve();
        }
        return this.checkIfPartOfEvent(user, eventID);
      }).then(() =>{
        // grab event
        resolve(event);
      }).catch((error) => {
  ...
  removeEvent(options){
    ...
    return new Promise(function(resolve, reject){
      checkIfUserHasAccess(manager, eventID).then(function(){
        ...
        // remove all files associated with this event too
    ...
    return new Promise(function(resolve, reject){
      checkIfUserHasAccess(options.user, options.eventID).then(()=>{
        ...
        if(options.checkedIn !== undefined){
          options.checkedIn = options.checkedIn === 'true';
        }
        let status = options.inviteStatus;
        if(status !== undefined
          && (status !== 'maybe' || status !== 'no' || status !== 'yes' || status != 'unknown')){
            return reject(errorResponses.INVALID_INVITE_STATUS);
        }
        ...
        if(error.type === eventErrorTypes.GENERIC_ERROR)
          error = errorResponses.UNABLE_TO_RETRIEVE_CHECKINS_ERROR;
        reject(error);
      });
    });
  }
    ...
    return new Promise(function(resolve, reject){
      ...
      validateEventIDAndCheckIn({
            ...
            if(saveError)
              ...
              notificationService.sendNotification({
                ...
                'notificationMessage' : {
                  ...
                  'action' : notificationConstants.NEW_EVENT_MESSAGE,
        ...
        if(error.type === eventErrorTypes.UNABLE_TO_RETRIEVE_CHECKIN_ERROR){
          console.log('Unable to retrieve checkin');
   ...
   *
   * @param {object} user - the user that is performing the action
   * @param {string} eventID - the id of the event
   * @param {number} options.limit - number of events to limit
   * @param {number} options.page - the 'page' to grab
   *
  ...
  removeEventManager(options){
    ...
    return new Promise(function(resolve, reject){
      ...
      checkIfUserHasAccess(options.user, options.eventID).then(() => {
        ...
        event = retrievedEvent;
        return self.getEventManagers({
          'eventID' : options.eventID,
          'user' : options.user
        });
      }).then((results) => {
        console.log('got the managers');
        if(results.length === 1){
          return reject(errorResponses.LAST_MANAGER);
        }
        return eventDao.getCheckIn({
          'orgGuid' : options.user.orgGuid,
          'personID' : options.managerID,
          'eventID' : options.eventID,
          'manager' : true
        });
      }).then((manager) => {
        manager.eventManager = false;
        manager.save((error, result) => {
          if(error){
            reject(errorResponses.UNABLE_TO_REMOVE_MANAGER);
          }
          else{
            elasticService.upsertCheckIn({
              'event' : event,
            ...
            cachingService.getSessionData(options.user.access_token).then((data)=> {
              console.log(data);
            });
            resolve({'res' : 'Manager removed successfully'});
          }
        });
      }).catch((error) => {
        console.log(error);
        let rejectResponse = error;
        if(error === undefined){
          rejectResponse = errorResponses.FORBIDDEN;
        }
        else if(error.type === eventErrorTypes.UNABLE_TO_RETRIEVE_CHECKIN_ERROR){
          rejectResponse = errorResponses.UNABLE_TO_REMOVE_MANAGER;
        }
        console.log(error);
        console.log(error);
        reject(rejectResponse);
    ...
    return new Promise(function(resolve, reject){
      ...
      if(eventID === undefined || personID === undefined){
        ...
        checkIfUserHasAccess(manager, eventID).then(function(){
          ...
          if(!checkIn.eventManager){
            checkIn.remove((err, result) => {
              console.log('remove returned');
              if(err){
                console.log(err);
                reject(errorResponses.UNABLE_TO_REMOVE_ATTENDEE);
              }
              else{
                elasticService.deleteCheckIn(checkIn.id).then(()=>{
                  console.log('successfully removed');
                }).catch(() => {
                  console.log('failed to remove');
                });
                resolve({'res' : 'Successfully removed attendee from event'})
              }
            });
          }
          else{
      ...
      validateEventIDAndCheckIn({
        ...
        return new Promise((innerResolve, innerReject) => {
          ...
          if(!event.private){
            ...
            eventDao.insertCheckIn(validationResult.validatedCheckIn).then(function(insertResult){
              checkInResult = insertResult.toObject();
              checkInResult.event = event.id;
              console.log('resolved!');
              innerResolve({'res' : 'User checked in successfully.'});
            }).catch(function(insertError){
              ...
              eventDao.getCheckIn({
                ...
                if(oldCheckIn.checkedIn === true){
                  ...
                  return innerReject(errorResponses.ALREADY_CHECKED_IN);
                }
                else{
                  ...
                  oldCheckIn.save(function(saveError, saveResponse){
                    if(saveError)
                      return innerReject(errorResponses.UNABLE_TO_UPDATE_CHECK_IN_STATUS);
                    else{
                      checkInResult =  saveResponse.toObject();
                      checkInResult.event = event.id;
                      innerResolve({'res' : 'User checked in successfully.'});
                    }
          ...
          else{ // it is a private event, so just grab the checkIn
            ...
            eventDao.getCheckIn({
              ...
              console.log(error);
              return innerReject(errorResponses.NOT_ATTENDEE);
            });
          }
        });

      }).then((result) => {
        console.log('got to resolving');
        console.log(event);
        elasticService.upsertCheckIn({
        ...
        notificationService.sendNotification({
          ...
          'notificationMessage' : {
            'type' : notificationConstants.EVENT,
            'action' : notificationConstants.NEW_CHECKIN,
            'content' : checkInResult
          }
        }).then(()=>{console.log('sent the message for the check in')}).catch(()=>{}); // not important
        resolve(result);
      }).catch(function(error){
        console.log('broken');
        console.log(error);
        ...
        result.forEach((event) => {
          ...
          bulkOps.push(new Promise((resolveInner, rejectInner) => {
            ...
            checkIfUserHasAccess(options.user, event.id).then(() => {
              ...
              Promise.all(promises.map((promise) => {
                ...
                if(process.env.NODE_ENV === 'test' || personIDs.length === 0)
                  return Promise.resolve();
                ...
                console.log('reisljsfel')
                console.log(results);
                console.log('got the real checkIns')
                let realCheckIns = [];
                let promise;
         ...
         Promise.all(bulkOps.map((promise) => {
           ...
           if(promise.isFulfilled()){
             ...
             console.log(val);
             response.results[events[index].id].successfulCheckIns =
                val.nUpserted + val.nInserted + val.nModified;
              if(response.results[events[index].id].successfulCheckIns > 0){
                console.log('got to this point');
                notificationService.sendNotification({
                  'store' : false,
                  'user' : options.user,
                  'event' : events[index].id,
                  'channel' : events[index].id,
                  'notificationMessage' : {
                    'type' : notificationConstants.EVENT,
                    'action' : notificationConstants.BULK_CHECKIN,
                    'content' : {
                      'successfulCheckIns' : response.results[events[index].id].successfulCheckIns
                    }
                  }
                }).then(()=>{
                  console.log('sent the message for the check in');
                }).catch(()=>{});
              }
  ...
  removeEventMessage(options){
    return new Promise((resolve, reject) => {
      let message, event;
      notificationService.getNotification(options.messageID).then((notification) => {
        console.log('Got notification');
        console.log(notification)
        event = notification.toObject().event;
        if(notification.message.type === notificationConstants.EVENT){
          message = notification;
          return checkIfUserHasAccess(options.user, event);
        }
        else{
          reject(errorResponses.FORBIDDEN);
        }
        ...
        message.remove((err, doc) => {
          ...
          return notificationService.sendNotification({
            ...
            'notificationMessage' : {
              'type' : notificationConstants.EVENT,
              'action' : notificationConstants.EVENT_MESSAGE_REMOVED,
              'content' : options.messageID
            }
          });
          resolve({'res' : 'Message was successfully removed'});
        });
      }).catch((error) => {
        console.log(error);
        reject(errorResponses.FORBIDDEN);
  ...
  // options.user
  // options.eventID
  // options.managerArray - array of personIDs
  ...
  retrieveCheckInIntervals(options){
    return new Promise((resolve, reject) => {
      console.log(options);
      checkIfUserHasAccess(options.user, options.eventID).then(() => {
        return eventDao.retrieveCheckInIntervals(
           options.eventID
        );
      }).then((dataset) => {
        ...
        // let dataset = intervals.reduce((prev, current) => {
        //   console.log(prev);
        //   console.log(current);
        //   if(prev !== undefined){

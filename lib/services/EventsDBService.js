...
class EventsDBService{
    ...
    return new Promise(function(resolve, reject){
      validateEvent(eventData, creatorData).then(function(event){
        ...
        eventDao.insertEvent(event).then(function(eventResult){
          ...
          eventDao.insertCheckIn(manager).then(function(checkInRes){
            checkInResult = checkInRes;
            // insert into elastic
            console.log(eventResult);
            return elasticService.insertEvent(eventResult.toObject());
          }).then(function(elasticResult){
            ...
            eventDao.removeEvent(eventResult._id).then(function(result){
              return elasticService.removeEvent(eventResult._id);
            }).then(function(result){
    ...
    return new Promise(function(resolve, reject){
      if(eventData.eventID === undefined){
        ...
        retrieveEvent(eventData.eventID).then(function(retrievedEvent){
          ...
          elasticService.updateEvent(event).then(function(result){
  ...
  removeEvent(eventID, managerData){
    return new Promise(function(resolve, reject){
      checkIfManager(managerData, eventID).then(function(){
        ...
        // remove the event
        elasticService.removeEvent(eventID).then(function(){
          console.log('Removed event');
        }).catch(function(error){
          console.log('unable to remove event');
        });
   ...
   *  @param {boolean} options.private - whether or not the event is private or not
   *  @param {number} options.limit - the number of pages to limit by
   *  @param {number} options.page - the page to grab
   *
   *  Returns a promise that contains the search results
   */
  searchEvents(options){
    return new Promise(function(resolve, reject){
      console.log(options);
      // if private or getting events a user is managing, do an application-side join
      if(options.private === 'true' || options.managing === true){
        // get array of events for term matching
        console.log('got here');
        eventDao.getEventIDs({
          'personID' : options.user.personID,
          'orgGuid' : options.user.orgGuid,
          'manager' : true,
        }).then(function(results){
          // pass it to elastic
          return elasticService.searchEvents({
            'query' : options.query,
            'limit' : options.limit,
        ...
        elasticService.searchEvents({
          ...
          reject({
            'status' : 500,
            'error_msg' : 'could execute the search'
          });
        })
      }
    });

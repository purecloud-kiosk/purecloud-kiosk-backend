...
function validateEventIDAndCheckIn(options){
  return new Promise(function(resolve, reject){
    ...
    if(eventID === undefined || checkIn === undefined){
      ...
      checkIfManager(manager, eventID).then(function(){
    ...
    return new Promise(function(resolve, reject){
      ...
      validateEvent(eventData, creatorData).then(function(event){
        ...
        var manager = {
          ...
          'eventLocation' : eventResult.location,
          'eventPrivate' : eventResult.private,
        ...
        elasticService.insertCheckIn(checkInResult.toObject()).then(function(done){
          console.log(done);
        }).catch(function(error){
          console.log(error);
        });
    ...
    return new Promise(function(resolve, reject){
      ...
      if(eventData.eventID === undefined){
        ...
        retrieveEvent(eventData.eventID).then(function(retrievedEvent){
          ...
          return checkIfManager(manager, eventData.eventID);
        }).then(function(){
          ...
          if(event.private === true && eventData.private === false){
            ...
            inviteDao.removeInvitationsByEventID(eventData.eventID).then(function(result){
            }).catch(function(error){
              console.log(error); // shouldnt happen
            });
          }
  ...
  removeEvent(options){
    ...
    return new Promise(function(resolve, reject){
      checkIfManager(manager, eventID).then(function(){
        ...
        elasticService.deleteCheckInsByEventID(eventID).then(function(result){
        }).catch(function(error){
          console.log(error);
        });
      ...
      validateEventIDAndCheckIn({
        ...
        if(validationResult.event.private === false){
          ...
          eventDao.insertCheckIn(validationResult.validatedCheckIn).then(function(insertResponse){
            elasticService.insertCheckIn(insertResponse.toObject()).then(function(response){
            }).catch(function(error){
              console.trace(error);
            });
      ...
      validateEventIDAndCheckIn({
        ...
        if(checkIn.eventManager){ // if already a manager, let them know
          ...
          checkIn.save(function(saveError, saveResponse){
            if(saveError)
              ...
              var savedCheckIn = saveResponse.toObject();
              savedCheckIn.event = savedCheckIn.event._id;
              elasticService.updateCheckIn(savedCheckIn).then(function(response){
              }).catch(function(error){
                console.trace(error);
              });
            }
        ...
        if(error.type === eventErrorTypes.UNABLE_TO_RETRIEVE_CHECKIN_ERROR){
          eventDao.insertCheckIn(validationResult.validatedCheckIn).then(function(insertResult){
            resolve({'res' : 'User had been made a manager'})
            elasticService.insertCheckIn(insertResult.toObject()).then(function(response){
            }).catch(function(error){
              console.trace(error);
            });
          }).catch(function(insertError){
          ...
          eventDao.insertCheckIn(validationResult.validatedCheckIn).then(function(insertResult){
            ...
            eventDao.getCheckIn({
              ...
              if(oldCheckIn.checkedIn === true){
                return reject(errorResponses.ALREADY_checkedIn);
              }
              else{

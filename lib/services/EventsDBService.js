...
class EventsDBService{
    ...
    return new Promise(function(resolve, reject){
      if(eventData.eventID === undefined){
        ...
        retrieveEvent(eventData.eventID).then(function(retrievedEvent){
          ...
          return reject(error); // default to error from retrieve or validate
        });
      }
    });
  }
  ...
  removeEvent(eventID, managerData){
    return new Promise(function(resolve, reject){
      checkIfManager(managerData, eventID).then(function(){
        ...
        return resolve(error); // default to checkInManager error
      });
    });
  }
  /**
  ...
  getPublicEvents(orgGuid, options){
    return new Promise(function(resolve, reject){
      setPaginationOptions(options).then(function(optns){
        ...
        return reject(error) // default to paging error
      });
    });
  }
        ...
        dao.getCheckIn({
          ...
          'eventID' : eventID
            ...
            if(checkIn.event_manager){ // if already a manager, let them know
              return reject(errorResponses.USER_ALREADY_A_MANAGER);
            }
            else{ //update their manager status
              checkIn.event_manager = true;
              checkIn.save(function(saveError, saveResponse){
                if(saveError)
                  return reject(errorResponses.UNABLE_TO_MAKE_USER_A_MANAGER);
                else
   ...
   *
   * @return Returns a promise that resolves when the user has been removed
   **/
  removeAttendee(eventID, managerData, personID){
    return new Promise(function(resolve, reject){
      if(eventID === undefined || personID === undefined){
        return reject(errorResponses.EVENT_ID_AND_OR_PERSON_ID_MISSING);
      }
      else if(managerData.personID === personID){
        return reject(errorRespones.MANAGER_CANNOT_REMOVE_THEMSELF)
      }
      else {
        checkIfManager(managerData, eventID).then(function(){
          dao.removeCheckIn(personID, eventID).then(function(removeResult){
            ...
            return reject(errorResponses.UNABLE_TO_REMOVE_EVENT);
          });
        }).catch(function(error){
          return reject(error);
        });
        ...
        if(validationResult.event.private === false){ // string because of redis
          ...
          dao.getCheckIn({
            ...
            if(checkIn.checked_in === true){
              ...
              checkIn.save(function(saveError, saveResponse){
                if(saveError)
                  return reject(errorResonses.UNABLE_TO_UPDATE_CHECK_IN_STATUS);
                else
                  return resolve({'res' : 'User checked in successfully.'});
              });
            }

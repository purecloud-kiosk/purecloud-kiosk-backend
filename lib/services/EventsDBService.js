...
class EventsDBService{
    ...
    if(eventID === undefined || checkInData === undefined){
      ...
      dao.getCheckIn(managerData.personID, eventID, function(error, result){
        if(error){
          ...
          callback({"error" : "Only one of the event's managers can manage check ins"});
        }
        else {
  ...
  getEvent(eventID, managerData, callback){
    dao.getCheckIn(managerData.personID, eventID, function(error, result){
      if(error){
        ...
        callback({"error" : "Only an event's manager can update an event"});
      }
      else {
        dao.getEvent(eventID, function(getEventError, event){
          if(getEventError){
            callback({"error" : "Error while retrieving event"});
          }
          else{
            callback(null, event);
          }
  ...
  updateEvent(eventData, managerData, callback){
    if(eventData.eventID === undefined){
      ...
      dao.getCheckIn(managerData.personID, eventData.eventID, function(error, result){
        if(result === null){
          callback({"error" : "Only an event's manager can update an event"});
        }
        else{
  ...
  removeEvent(eventID, managerData, callback){
    dao.getCheckIn(managerData.personID, eventID, function(error, result){
      if(result === null){
        callback({"error" : "Only an event's manager can remove an event"});
      }
      else{
        dao.removeEvent(eventID, function(removeError, removeResult){
          if(removeResult.n == 1 && removeResult.ok == 1){
            callback(error, {"res" : "Event was successfully removed"});
          }
          else{
            callback(true, {"res" : "Event does not exist"});
          }
        });
      }
    });

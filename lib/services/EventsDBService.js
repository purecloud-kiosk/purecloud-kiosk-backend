...
var eventFields = ["location", "image_url", "thumbnail_url"];
/**
...
function validateEvent(eventData, creatorData, callback){
  // check all of the fields that are default and check to see if
  ...
  if(eventData.title === undefined ||
    ...
    // add validation on illegal chars in title here.
    ...
    if(!validator.isDate(eventData.date) && !validator.isNumeric(eventData.date)){
      console.log("is it a date? " + validator.isDate(eventData.date) );
      console.log("is it a number? " + validator.isNumeric(eventData.date));
    ...
    checkIfManager(managerData, eventID, function(error){
      if(error){
        ...
        if(checkInData.personID === undefined ||
          ...
          return callback({"error" : "CheckIn requires a 'personID', 'name', 'organization', and 'timestamp'"});
        }
        else{
...
class EventsDBService{
      ...
      if(error){
        ...
        dao.insertEvent(event, function(eventError, eventResult){
          if(eventError){
            ...
            return callback({"error" : "An event with the name " + event.title + " under the organization " + event.organization +
              " already exists."});
          }
          else {
            var manager = {
   ...
   **/
  getPrivateEvents(personID, options, callback){
    setPaginationOptions(options, function(error, limit, page){
      if(error){
        return callback(error);
      }
      dao.getPrivateEvents(personID, limit, page, function(error, result){
        if(error){
          return callback({"error" : "Bad request. Could not retrieve events."});
        }
        else{

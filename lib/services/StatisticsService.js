...
class StatisticsService{
  ...
  getEventStats(options){
    ...
    return new Promise((resolve, reject) => {
      eventService.fetchEvent(options.user, options.eventID).then((retrievedEvent) => {
        stats.userIsManager = true;
        eventService.checkIfManager(options.user, options.eventID).then(()=>{
          return new Promise.resolve();
        }).catch(() => {
          stats.userIsManager = false;
          console.log('got to this catch');
          return new Promise.resolve();
        }).then(() => {
          return eventDao.getAttendanceCounts(options.eventID);
        }).then((results) => {
          if(results.length === 0)

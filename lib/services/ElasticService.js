...
class ElasticService{
  ...
  getEvents(options){
    return new Promise(function(resolve, reject){
      ...
      var before = new Date(options.before).getTime();
      var after = new Date(options.after).getTime();
      if(after >= before){
        reject('error');
      }
      else{
        elasticDao.getEvents({
          'before' : new Date(options.before),
          'after' : new Date(options.after),
          'user' : options.user
        }).then(function(results){
          var events = [];
          results.aggregations.dedup.buckets.forEach(function(element){
            var event = element.dedup_docs.hits.hits[0]._source;

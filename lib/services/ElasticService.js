...
class ElasticService{
  ...
  searchEvents(options){
    return new Promise(function(resolve, reject){
      var limit = 25, page = 0, searchFields = [];
      ...
      elasticDao.searchEvents({
        ...
        if(result.aggregations.eventIDs.buckets.length > 0){
          ...
          console.log(hits);
          var eventIDs = [];
          for(var i = (page * limit); i < (page * limit + limit) && i < hits.length; i++){
        ...
        for(var i = 0; i < events.length; i++){
          events[i].startDate = new Date(events[i].startDate).getTime();
          events[i].endDate = new Date(events[i].endDate).getTime();
        }
        console.log(events);
        resolve(events);
      }).catch(function(error){

...
class PureCloudAPIService{
      ...
      self.getSession(accessToken).then(function(requestResult){
        ...
        cachingService.storeSessionData({
          ...
          'sessionData' : {
            ...
            'image' : userImage,
            'userType' : data.res.user.userType,
            'eventsManaging' : []
          },
  ...
  /**
   *  Bulk retrieves multiple people belonging to the org via a personID
   *
   *  @param {string} options.personIDs - the person ids seperated by commas
   *  @param {string} options.access_token
   *
   *  @return Returns a promise the resolves upon a successfully getting a user's session data.
   *  Rejects if the user is not authenticated with PureCloud.
   **/
  bulkRetrievePeople(options){
    return new Promise(function(resolve, reject){
      request(genRequestOptions({
        'url' : 'https://apps.mypurecloud.com/api/v2/people/bulk/' + options.personIDs,
        'method' : 'GET',
        'token' : options.access_token
      }), function(error, response, body){
        if(error || response.statusCode !== 200)
          reject(); // doesn't matter what the error was, got rejected
        else
   ...
   *  Bulk retrieves multiple people belonging to the org via a personID
   *
   *  @param {string} options.personID - the person ids seperated by commas
   *  @param {string} options.access_token
   *
   *  @return Returns a promise the resolves upon a successfully getting a user's session data.
   *  Rejects if the user is not authenticated with PureCloud.
   **/
  retrievePerson(options){
    return new Promise(function(resolve, reject){
      return request(genRequestOptions({
        'url' : 'https://apps.mypurecloud.com/api/v2/people/' + options.personID +
          '?fl=general,images,primaryContactInfo',
        'method' : 'GET',
        'token' : options.access_token
      }), function(error, response, body){
        if(error || response.statusCode !== 200)
          reject(errorResponses.PERSON_DOES_NOT_EXIST); // doesn't matter what the error was, got rejected
        else
          resolve(JSON.parse(body));
      });
    });

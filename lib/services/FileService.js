...
var fileErrorTypes = require('lib/models/errors/fileDaoErrorTypes');
...
function uploadToBucket(options){
  return new Promise((resolve, reject) => {
    console.log('made it to s3 upload call')
...
class FileService{
  ...
  removeFile(options){
    return new Promise((resolve,reject) => {
      ...
      fileDao.getFile({
        ...
        file = retrievedFile;
        if(file.fileType === 'eventFile'){
          return eventService.checkIfUserHasAccess(options.user, file.toObject().event);
        }
        else if(file.uploaderID === options.user.personID){
   ...
   **/
  removeAllEventFiles(eventID){
    return new Promise((resolve,reject) => {
      let files = [];
      fileDao.getEventFiles(eventID).then((retrievedFiles) => {
        console.log("this many " + retrievedFiles.length);
        if(retrievedFiles.length > 0){
          console.log('there are files');
          retrievedFiles.forEach((file) => {
            console.log( file.url.substring(getNthCharIndex(file.url, '/', 3) + 1, file.url.length));
            files.push({
              'Key' : file.url.substring(getNthCharIndex(file.url, '/', 3) + 1, file.url.length)
            });
          });
          fileDao.removeEventFiles(eventID).then(() => {

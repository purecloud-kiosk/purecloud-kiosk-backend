...
router.get("/getAttendees", function(req, res){
 ...
 router.post("/addAttendee", function(req, res){
   eventsService.addAttendee(req.body.eventID, req.user, req.body.checkIn, function(error, result){
     if(error){
       res.status(400).send(error);
     }
     else{
...
router.post("/checkIn", function(req, res){
 ...
 /**
  * Route for uploading images, still needs to store image url in DB for later access
  **/

...
router.post('/create', function(req, res){
  ...
  eventsService.createEvent(req.body, req.user).then(function(result){
    res.send(result);
...
router.post('/update', function(req, res){
  eventsService.updateEvent(req.body, req.user).then(function(result){
      res.send(result);
  }).catch(function(error){
      res.status(400).send(error);
  });
...
router.post('/remove', function(req, res){
  eventsService.removeEvent(req.body.eventID, req.user).then(function(result){
    res.send(result);
  }).catch(function(error){
    res.status(400).send(error);
  });
...
router.get('/managing', function(req, res){
  eventsService.getEventsManaging(req.user.personID, req.user.orgGuid, {
...
router.get('/attending', function(req, res){
  eventsService.getEventsAttending(req.user.personID, req.user.orgGuid, {
    'limit' : req.query.limit,
    'page' : req.query.page
  }).then(function(result){
    res.send(result);
...
router.get('/public', function(req, res){
  ...
  eventsService.getPublicEvents(req.user.orgGuid, {
    'limit' : req.query.limit,
    'page' : req.query.page
  }).then(function(result){
    res.send(result);
...
router.get('/private', function(req, res){
  eventsService.getPrivateEvents(req.user.personID, req.user.orgGuid, {
    'limit' : req.query.limit,
    'page' : req.query.page
  }).then(function(result){
    res.send(result);
...
router.get('/getEventCheckIns', function(req, res){
 ...
 router.get('/searchManagedEvents', function(req, res){
   eventsService.searchManagedEvents(req.query.q, req.user, {
     'limit' : req.query.limit,
     'page' : req.query.page
   }).then(function(result){
     res.send(result);
 ...
 router.post('/addPrivateAttendee', function(req, res){
   eventsService.addPrivateAttendee(req.body.eventID, req.user, req.body.attendee).then(function(result){
     res.send(result);
   }).catch(function(error){
     res.status(400).send(error);
   });
 ...
 router.post('/addEventManager', function(req, res){
   eventsService.addEventManager(req.body.eventID, req.user, req.body.manager).then(function(result){
     res.send(result);
   }).catch(function(error){
     res.status(400).send(error);
   });
 ...
 router.post('/removePrivateAttendee', function(req, res){
   eventsService.removeAttendee(req.body.eventID, req.user, req.body.personID).then(function(result){
     res.send(result);
   }).catch(function(error){
     res.status(400).send(error);
   });
...
router.post('/checkIn', function(req, res){
  eventsService.checkIntoEvent(req.body.eventID, req.user, req.body.checkIn).then(function(result){
    res.send(result);
  }).catch(function(error){
    res.status(400).send(error);
  });
 ...
 router.post('/uploadImage', busboy({'immediate' : true}), function(req, res){
   ...
   req.busboy.on('file', function(fieldname, file, filename, encoding, mimeType){
     ...
     if(fieldname != 'file'){
       ...
       fileAvailable = true;
       uploadService.uploadImage(req.user, filename, file, encoding, mimeType, function(error, result){
        complete = true;
        if(error){
          res.status(400).send(error);
        }

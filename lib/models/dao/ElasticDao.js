...
class ElasticDao {
  ...
  searchEvents(options){
    return new Promise((resolve, reject) => {
      ...
      console.log((options.limit * options.page) + options.limit);
      var searchOptions = {
        'index' : 'eventdb',
        'body' : {
          'query' : {
      ...
      if(options.managing === 'true'){
        ...
        searchOptions.body.query.bool.should[0].bool.filter = searchOptions.body.query.bool.should[0].bool.filter.slice(1,1);
        searchOptions.body.query.bool.should[0].bool.filter.push({
          'match' : {
            'private' : options.private
          }

  ...
  insertFile(fileData){
    return new Promise((resolve, reject) => {
      ...
      file.save((err, fileSaved) => {
        console.log(err);
        if(err)
          reject(new CustomError(error, errorTypes.UNABLE_TO_INSERT_FILE));
        else
          resolve(fileSaved);
      });
    });
  ...
  removeFile(fileID){
    return new Promise((resolve, reject) => {
      File.remove({'_id' : fileID}, (error, result) => {
        if(error)
          reject(new CustomError(error, errorTypes.UNABLE_TO_REMOVE_FILE));
        else
          resolve(result);
      });
    });
    ...
    return new Promise((resolve, reject) => {
      ...
      File.findOne({'_id' : options.fileID, 'uploaderID' : options.uploaderID}).exec((error, result) => {
        if(error || result === null || result === undefined)
          reject(new CustomError(error, errorTypes.UNABLE_TO_RETRIEVE_FILE));
        else
          resolve(result);
      });
    });
  ...
  getUploaderFiles(options){
    return new Promise((resolve, reject) => {
      ...
      .skip(options.limit * options.page).sort({'uploadDate' : 1}).exec((error, results) => {
        if(error)
          reject(new CustomError(error, errorTypes.UNABLE_TO_RETRIEVE_FILES));
        else
          resolve(results);
      });
    });

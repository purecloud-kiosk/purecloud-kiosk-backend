...
class EventDao {
    ...
    return new Promise(function(resolve, reject){
      CheckIn.findOne(optns).populate('event')
        .exec(function(error, result){
          if(error || result === null || result === undefined)
            reject(new EventDaoError(error, eventErrorTypes.UNABLE_TO_RETRIEVE_CHECKIN));
          else
            resolve(result);
        });
    });
  ...
  getAssociatedEvents (options){
    return new Promise(function(resolve, reject){
      CheckIn.find({'personID' : options.personID, 'orgGuid' : options.orgGuid, 'eventManager' : options.manager})
        ...
        .sort({'date' : -1}).skip(options.limit * options.page).lean().exec(function(error, result){
          if(error || result === null || result === undefined )
            reject(new EventDaoError(error, eventErrorTypes.UNABLE_TO_RETRIEVE_EVENTS));
          else
            resolve(result);
        });
    });
  ...
  getAttendanceCounts(eventID){
    return new Promise(function(resolve, reject){
      CheckIn.aggregate([
        ...
        if(error || result === null || result === undefined )
          reject(new EventDaoError(error, eventErrorTypes.UNABLE_TO_GET_COUNT));
        else
          resolve(result);
      });
    });
  ...
  getEventCheckIns(options){
    return new Promise(function(resolve, reject){
      CheckIn.find({'event' : options.eventID, 'checked_in' : true}).select('-eventManager -event -id')
  ...
  removeUnattendedCheckInsByEvent(eventID){
    return new Promise(function(resolve, reject){
      CheckIn.remove({'event' : eventID, 'eventManager' : false, 'checked_in' : false}, function(error, result){
        if(error)
          reject(new EventDaoError(error, eventErrorTypes.REMOVE_CHECKIN_ERROR));
        else
          resolve(result);
  ...
  // retrieves all event IDs that from a checkin under an org
  getEventIDs(options){
    return new Promise(function(resolve, reject){
      CheckIn.find({
      ...
      }).distinct('event', function(error, ids){

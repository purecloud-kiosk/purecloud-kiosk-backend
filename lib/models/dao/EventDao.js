...
class EventDao {
  ...
  getEvent(eventID){
    return new Promise(function(resolve, reject){
      Event.findOne({'_id' : eventID}).select('-_id').lean().exec(function(error, result){
    ...
    return new Promise(function(resolve, reject){
      CheckIn.find({
        ...
        if(options.upcoming === true){
          eventOptions.startDate = {'$gte' : new Date()};
        }
        Event.find(eventOptions).select('-_id').lean().limit(options.limit)
          .skip(options.limit * options.page).sort({'startDate' : -1}).exec(function(error, results){
            resolve(results);
        });
      });
    });
  ...
  getPublicEvents(options){
    return new Promise(function(resolve, reject){
      ...
      if(options.upcoming === true)
        eventOptions.startDate = {'$gte' : new Date()}
      Event.find(eventOptions).limit(options.limit).select('-_id')
      .sort({'startDate' : 1}).skip(options.limit * options.page).lean().exec(function(error, result){
        if(error || result === null || result === undefined)
  ...
  getPrivateEvents(options){
    return new Promise(function(resolve, reject){
      ...
      .distinct('event').lean().exec(function(error, results){
        ...
        Event.find(eventOptions).select('-_id').sort({'startDate' : 1}).limit(options.limit)
        .skip(options.limit * options.page).lean().exec(function(error, events){
          //console.log(events);
          resolve(events);
        });
      });
  ...
  /**
   *  Returns all private events in an org. Used by org admins.
   **/
  getAllPrivateEvents(options){
    return new Promise(function(resolve, reject){
      var eventOptions = {
        'orgGuid' : options.orgGuid,
        'private' : true
      ...
      Event.find(eventOptions).select('-_id').sort({'startDate' : 1}).limit(options.limit)
      .skip(options.page * options.limit).lean().exec(function(error, events){
        resolve(events);
      });
    });
  }
  ...
  searchManagedEvents(options){
    return new Promise(function(resolve, reject){
      ...
      .distinct('event').lean().exec(function(error, results){
        ...
        if(options.upcoming === true)
          eventOptions.startDate = {'$gte' : new Date()}
        Event.find(eventOptions).sort({'startDate' : 1}).limit(options.limit).skip(options.limit * options.page).lean().exec(function(error, events){
          resolve(events);
        });
      });
    });
  ...
  getCheckInIDsByEvent(eventID){
    return new Promise(function(resolve, reject){
      console.log(eventID);
      CheckIn.find({'event' : eventID}).distinct('_id').lean().exec(function(error, result){
        resolve(result);
      })
    });
  }
  ...
  getEventCheckIns(options){
    return new Promise(function(resolve, reject){
      CheckIn.find({'event' : options.eventID, 'checkedIn' : true}).select('-event')
        .limit(options.limit).skip(options.limit * options.page).sort({'timestamp' : 1}).exec(function(error, result){
      ...
      CheckIn.aggregate([
        ...
        {'$group' : {
         '_id' : {
           ...
           'startDate' : {'$gte' : ['$event.startDate', new Date()]}
         },
         'total' : {'$sum' : 1 }
        }}
      ], function(error, result){
  ...
  populateEvents(options){
    return new Promise(function(resolve, reject){
      ...
      Event.find(eventOptions).select('-_id').lean().exec(function(error, result){
        if(error)
          reject(new EventDaoError(error, eventErrorTypes.UNABLE_TO_RETRIEVE_EVENTS_ERROR));
        else{
          // sort to match original input array order
          var res = result.sort(function(a, b) {
            return options.eventIDs.indexOf(a.id.toString()) - options.eventIDs.indexOf(b.id.toString());
          });
          resolve(res);
        }
      });

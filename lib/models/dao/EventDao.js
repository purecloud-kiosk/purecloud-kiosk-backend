  ...
  getPrivateEvents(options){
    return new Promise(function(resolve, reject){
      ...
      .distinct('event').lean().exec(function(error, results){
        ...
        if(options.upcoming === true)
          eventOptions.date = {'$gte' : new Date()}
        Event.find(eventOptions).select('-_id').sort({'date' : 1}).limit(options.limit).skip(options.limit * options.page).lean().exec(function(error, events){
          //console.log(events);
          console.log(error);
          resolve(events);
        });
  ...
  populateEvents(options){
    return new Promise(function(resolve, reject){
      ...
      if(options.sortOrder === 'desc')
        sort = -1;
      console.log(sort);
      Event.find(eventOptions).select('-_id').sort({'date' : sort}).exec(function(error, result){
        if(error)
          reject(new EventDaoError(error, eventErrorTypes.UNABLE_TO_RETRIEVE_EVENTS_ERROR));
        else
          resolve(result);

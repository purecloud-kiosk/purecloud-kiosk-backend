...
class EventDao {
  ...
  getAssociatedEvents (personID, manager, limit, page, callback){
    CheckIn.find({"person_id" : personID, "event_manager" : manager})
      .select("event checked_in -_id").populate("event").limit(limit).skip(limit * page).lean().exec(callback);
  }
  /**
  ...
  getPublicEvents(organization, limit, page, callback){
    Event.find({"organization" : organization, "private" : false}).limit(limit).skip(limit * page).lean().exec(callback);
  }
  /**
  ...
  getCheckIns(personID, organization, callback){
    CheckIn.find({"person_id" : personID}).populate("event").exec(callback);
  }
  /**

...
class EventDao {
  ...
  getEvent(eventID){
    return new Promise((resolve, reject) => {
      Event.findOne({'_id' : eventID}).select('-_id').lean().exec((error, result) => {
  ...
  getEvents(eventIDs){
    return new Promise((resolve, reject) => {
      Event.find({'_id' : {'$in' : eventIDs}}).select('-_id').lean().exec((error, result) => {
        if(error || result === null || result === undefined)
          reject(new EventDaoError(error, eventErrorTypes.GENERIC_ERROR));
        else
          resolve(result);
    ...
    return new Promise((resolve, reject) => {
      CheckIn.find({
        ...
        Event.find(eventOptions).select('-_id').lean().limit(options.limit)
          .skip(options.limit * options.page).sort({'startDate' : -1}).exec((error, results) => {
            resolve(results);
        });
      });
    });
  ...
  getCheckIns(personID, orgGuid){
    return new Promise((resolve, reject) => {
      CheckIn.find({'personID' : personID, 'orgGuid' : orgGuid}).populate('event').exec((error, result) => {
        if(error || result === null || result === undefined )
          reject(new EventDaoError(error, eventErrorTypes.UNABLE_TO_RETRIEVE_CHECKIN_ERROR));
        else
          resolve(result);
  ...
  searchManagedEvents(options){
    return new Promise((resolve, reject) => {
      ...
      .distinct('event').lean().exec((error, results) => {
        ...
        if(options.upcoming === true)
          eventOptions.endDate = {'$gte' : new Date()}
        Event.find(eventOptions).sort({'startDate' : 1}).limit(options.limit)
          .skip(options.limit * options.page).lean().exec((error, events) => {
          resolve(events);
        });
      });
  ...
  getEventCheckIns(options){
    return new Promise((resolve, reject) => {
      CheckIn.find({'event' : options.eventID, 'checkedIn' : true}).select('-event')
        .limit(options.limit).skip(options.limit * options.page).sort({'timestamp' : 1}).exec((error, result) => {

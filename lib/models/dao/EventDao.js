...
class EventDao {
  ...
  updateEvent(options){
    return new Promise((resolve, reject) => {
      ...
      Event.findOneAndUpdate({'_id' : options.eventID}, options.eventData,{'new' : true}, (error, result) => {
        if(error){
          reject(new CustomError(error, eventErrorTypes.DUPLICATE_EVENT_ERROR));
        }
        else{
  ...
  getEventManagers(options){
    return new Promise((resolve, reject) => {
      let query = CheckIn.find({'event' : options.eventID, 'eventManager' : true}).select('-eventManager -event -_id -__v')
      if(!options.getAll){
  ...
  getEventCheckIns(options){
    return new Promise((resolve, reject) => {
      var query = CheckIn.find({'event' : options.eventID, 'checkedIn' : true}).select('-event -__v').sort({'timestamp' : 1});
      if(!options.getAll){
        query.limit(options.limit).skip(options.limit * options.page);
      }
      query.exec((error, result) => {
  ...
  getSelectCheckIns(options){
    return new Promise((resolve, reject) => {
      CheckIn.find({'event' : options.eventID, 'orgGuid' : options.orgGuid, 'personID' : {'$in': options.personIDs}})
        .select('-_id -__v').exec((error, result) => {
          if(error || result === undefined)
            reject(new CustomError(error, eventErrorTypes.GENERIC_ERROR));
          else
            resolve(result);
